...
const ParentNotification = () => {
  ...
  const fetchNotifications = async (page) => {
    try {
      ...
      console.log("Fetching parent notifications with token:", token);
      const response = await axios.get(
        `${BASE_URL}/api/notifications/role/parent?page=${page}&limit=${notificationsPerPage}`,
        {
      ...
      const errorMessage = err.response
        ? `Server error: ${err.response.status} - ${err.response.data?.error || 'Not Found'}`
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.header}>Parent Notifications ({notifications.length})</h2>
