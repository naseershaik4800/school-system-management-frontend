...
function StudentProgressParent() {
  ...
  // Parse user once using useMemo to avoid repeated parsing
  ...
  // Fetch progress data when user and selectedStudentId are available
  useEffect(() => {
    if (
      !user ||
      user.role !== 'parent' ||
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Please log in as a parent with associated children to view progress',
      }).then(() => navigate('/login'));
      return;
    }

    if (!selectedStudentId) {
      setSelectedStudentId(studentId || user.children[0]); // Use studentId first, fallback to first child
    } else {
      fetchProgressData(selectedStudentId);
    }
  }, [user, navigate, selectedStudentId, studentId]);
  ...
  const fetchProgressData = async (studentId) => {
    ...
    try {
      ...
      const studentRes = await axios.get(
        `${BASE_URL}/api/student/byId/${studentId}`
      );
      ...
      const [examsRes, attendanceRes, assignmentsRes, submittedAssignmentsRes] =
        await Promise.all([
          axios.get(`${BASE_URL}/api/exams/byStudent/${studentId}`),
          axios.get(`${BASE_URL}/api/student-attendance/${studentId}`),
          axios.get(`${BASE_URL}/api/student-assignments/${studentId}`),
          axios.get(`${BASE_URL}/api/submitted-assignments/${studentId}`),
        ]);
      ...
      exams.forEach((exam, index) => {
        ...
        if (studentMarks && studentMarks.marks.length > 0) {
          ...
          const examName = exam.name || `Exam ${index + 1}`;
          detailedExams.push({
            name: examName,
    ...
    return (
      <div className='container my-4 text-center'>
        <div className='alert alert-warning'>
          Please log in as a parent to view your child’s progress.
        </div>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className='text-center p-3'>Loading your child’s progress...</div>
    );
  }
    ...
    >
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered size='lg'>
        <Modal.Header closeButton className='bg-primary text-white'>
          <Modal.Title>My Child’s Exam Scores</Modal.Title>
        </Modal.Header>
...
export default StudentProgressParent;
